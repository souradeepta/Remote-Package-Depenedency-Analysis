...
// Authors:     Jim Fawcett, CST 2-187, Syracuse University              //
 ...
 * --------------------
 * ver 1.1 : 05 Dec 2018
 *   - new function added to format the dependency table output
 * ver 1.0 : 29 Nov 2018
 *   - first release
 * 
 */
...
namespace CodeAnalysis
{
  public class DependencyTable
  {
    public Dictionary<string, List<string>> dependencies { get; set; }
     ...
     public List<string> depFormat = new List<string>();
    //----< add parent file >----------------------------------------
    ...
    public void depDisplayFormat()
      {
            foreach (var item in dependencies)
            {
                string file = item.Key;
                file = System.IO.Path.GetFileName(file);
                depFormat.Add(file.ToString());
                if (item.Value.Count == 0)
                {
                    depFormat.Add(";");
                    continue;
